...
fn encrypt_bytes(source_filepath: &Path, bytes: String) -> () {
    ...
    //
    // Create Instruction_to_decode.txt file
    let instruction_to_decode_filepath =
        source_filepath.with_file_name("Instruction_to_decode.txt");
    let instruction_to_decode_file =
        File::create(&instruction_to_decode_filepath).unwrap_or_else(|err| {
            eprintln!("Failed to create output file: {}", err);
            process::exit(1);
        });
    let mut itd_writer = BufWriter::new(instruction_to_decode_file);

    let mut instruction_to_decode_vec: Vec<KeyFileData> = Vec::new();
    let mut line_number_current_fasta = 0;
    let lines = bytes.split_inclusive('\n');
    for line in lines {
        if line.starts_with("#") {
            if let Some(k) = instruction_to_decode_vec.last_mut() {
            ...
            instruction_to_decode_vec.push(key);
        } else {
    ...
    for item in instruction_to_decode_vec {
        if let Err(err) = writeln!(&mut itd_writer, "{}", item.to_instruction_to_decode()) {
            eprintln!("Failed to write to file: {}", err);
            process::exit(1);
        }
    }
